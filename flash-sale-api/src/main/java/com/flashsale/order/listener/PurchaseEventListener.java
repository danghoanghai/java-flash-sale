...
public class PurchaseEventListener {
    ...
    public void onPurchaseEvent(PurchaseEvent event) {
        log.info("Received PurchaseEvent | orderNo={} user={} fspId={}",
                event.getOrderNo(), event.getUserId(), event.getFlashSaleProductId());
            ...
            try {
                ...
                if (attempt < MAX_EVENT_RETRIES) {
                    try {
                        ...
                        log.error("Retry interrupted for order {}", event.getOrderNo());
                        return;
                    }
                }
            }
