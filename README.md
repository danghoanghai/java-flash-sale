...
│       (both with optimistic locking + retry)    │
└─────────────────────────────────────────────────┘
...
Seed data: 100 products across 5 flash sale campaigns (07-09, 10-12, 13-15, 16-18, 19-21), all on today's date.
...
| `inventory`          | Global stock per product (available_stock, version) |
| `flash_sale`         | Campaign (name, start/end time, status)            |
| `flash_sale_product` | Junction: product allocated to a sale (sale_price, sale_stock, sale_available, per_user_limit, version) |
