...
public interface FlashSaleProductRepository extends JpaRepository<FlashSaleProduct, Long> {
    ...
    @Modifying
    @Query("UPDATE FlashSaleProduct fsp SET fsp.saleAvailable = fsp.saleAvailable - 1, fsp.version = fsp.version + 1 " +
           "WHERE fsp.id = :id AND fsp.saleAvailable > 0 AND fsp.version = :version")
    int decrementSaleStock(@Param("id") Long id, @Param("version") Integer version);
}
